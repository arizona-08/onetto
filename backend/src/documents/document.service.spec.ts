...
const company = {
  name: 'Atelier Onetto',
  email: 'contact@onetto.test',
  phoneNumber: '0102030405',
  siren: '123456789',
  address: '10 rue Onetto',
  postalCode: '75002',
  city: 'Paris',
  country: 'France',
  vatNumber: 'FR123',
...
describe('DocumentService', () => {
  ...
  const bridge = {
    ...
    createPaymentLink: jest
      .fn()
      .mockResolvedValue({ url: 'https://pay.test/link' }),
  } as unknown as BridgeApiService;
  beforeEach(() => {
    jest.resetAllMocks();
    (mail.createInvoiceMail as jest.Mock).mockReturnValue({
      subject: 'Facture',
      text: 'facture',
      html: '<p>Facture</p>',
    });
    (mail.createInvoicePaymentRetryMail as jest.Mock).mockReturnValue({
      subject: 'Relance',
      text: 'relance',
      html: '<p>Relance</p>',
    });
    (mail.createEstimateMail as jest.Mock).mockReturnValue({
      subject: 'Devis',
      text: 'devis',
      html: '<p>Devis</p>',
    });
    (bridge.getCallbackUrl as jest.Mock).mockReturnValue(
      'https://callback.test',
    );
    (bridge.createPaymentLink as jest.Mock).mockResolvedValue({
    ...
    jest
      .spyOn(service as never, 'getActiveCompanyId')
  ...
  it('ne modifie que la date d’échéance d’une facture brouillon', async () => {
    ...
    expect(prisma.document.update).toHaveBeenCalledWith({
      where: { id: invoice.id },
      data: { paymentDueAt: new Date('2026-09-15') },
    });
  ...
  it('refuse de modifier une facture déjà envoyée', async () => {
    ...
    await expect(
      service.updateDraftDocument(invoice.id, dto, user),
  ...
  it('lie une facture créée au devis source avec sourceDocumentId', async () => {
    ...
    jest
      .spyOn(service, 'createDocumentNumber')
      .mockResolvedValue('#FACT-2026-0002');
  ...
  it('empêche une seconde conversion du même devis', async () => {
    ...
    await expect(
      service.convertEstimateToInvoice('estimate-1', user),
    ).rejects.toBeInstanceOf(BadRequestException);
  ...
  it('envoie une facture brouillon avec son PDF puis la marque SENT', async () => {
    ...
    expect(mail.createInvoiceMail).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('ne marque pas une facture comme envoyée si la génération PDF échoue', async () => {
    (prisma.company.findUnique as jest.Mock).mockResolvedValue(company);
    (pdf.generate as jest.Mock).mockRejectedValue(
      new Error('PDF indisponible'),
    );

    const consoleError = jest.spyOn(console, 'error').mockImplementation();
    await expect(
      service.sendDocumentToClient(invoice.id, user),
    ).rejects.toThrow(
  ...
  it('refuse un second envoi de facture', async () => {
    jest
      ...
      .mockResolvedValue({ ...invoice, invoiceStatus: 'PENDING' } as never);
  ...
  it('génère un nouveau lien et envoie un email de relance pour une facture rejetée', async () => {
    ...
    jest
      .spyOn(service, 'getDocumentById')
      .mockResolvedValue({ ...invoice, invoiceStatus: 'REJECTED' } as never);
    (prisma.company.findUnique as jest.Mock).mockResolvedValue(company);
    (pdf.generate as jest.Mock).mockResolvedValue(pdfBuffer);
    ...
    expect(mail.createInvoicePaymentRetryMail).toHaveBeenCalledWith(
      expect.objectContaining({
        paymentLink: expect.stringContaining('/payment?token='),
      }),
    );
    expect(mail.sendMail).toHaveBeenCalledWith(
      expect.objectContaining({
        attachments: [expect.objectContaining({ content: pdfBuffer })],
      }),
    );
    expect(prisma.document.update).toHaveBeenCalledWith({
      where: { id: invoice.id },
  ...
  it('refuse une relance si la facture n’est pas rejetée', async () => {
    await expect(
      service.retryInvoicePayment(invoice.id, user),
    ).rejects.toBeInstanceOf(BadRequestException);
  ...
  it('marque manuellement comme payée une facture en attente', async () => {
    ...
    await service.manuallyMarkInvoiceAsPaid(invoice.id, user, 'BANK_TRANSFER');
  ...
  it('envoie un devis avec un token de négociation et le template HTML dédié', async () => {
    ...
    (prisma.estimateNegociation.create as jest.Mock).mockResolvedValue({
      negociationToken: 'secure-token',
    });

    await service.sendDocumentToClient(estimate.id, user);

    expect(prisma.estimateNegociation.create).toHaveBeenCalled();
    expect(mail.createEstimateMail).toHaveBeenCalledWith(
      expect.objectContaining({
        documentUrl: expect.stringContaining('secure-token'),
      }),
    );
    expect(mail.sendMail).toHaveBeenCalledWith(
