...
function MyCompanyPage() {
  ...
  const [
    isGoCardlessAuthorizationPending,
    setIsGoCardlessAuthorizationPending,
  ] = React.useState(false);
  const [goCardlessAuthorizationError, setGoCardlessAuthorizationError] =
    React.useState<string | null>(null);
  const [isGoCardlessVerificationPending, setIsGoCardlessVerificationPending] =
    React.useState(false);

  React.useEffect(() => {
    async function loadCompanyDetails() {
      const companyResponse = await getCompany(params.id);
  ...
  async function startGoCardlessAuthorization() {
    ...
    if (!response.ok) {
      setGoCardlessAuthorizationError(
        "Impossible de démarrer la connexion à GoCardless. Réessayez dans quelques instants.",
      );
        ...
        <>
          <section className="mt-6 rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
            ...
            <dl className="mt-7 grid gap-x-8 gap-y-6 border-t border-zinc-100 pt-6 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <InformationItem
                label="Ville"
                value={`${company.postalCode} ${company.city}`}
              />
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm leading-6 text-zinc-700">
                      Créez puis connectez un compte GoCardless pour permettre à
                      vos clients de vous régler et recevoir vos paiements.
                    </p>
                    <button
                      type="button"
                    ...
                    >
                      {isGoCardlessAuthorizationPending
                        ...
                        : "Créer ou connecter mon compte GoCardless"}
                    </button>
              ...
              company.companyPaymentAccount?.verificationStatus ===
                ...
                <div className="mt-6 rounded-lg border border-orange-100 bg-orange-50/70 p-5">
                  ...
                  <p className="mt-2 text-sm text-zinc-600">
                    Veuillez vérifier votre compte GoCardless pour commencer à
                  ...
                  >
                    {isGoCardlessVerificationPending
                      ? "Vérification en cours…"
                      : "Vérifier mon compte GoCardless"}
                  </button>
                  <button
                    type="button"
                    onClick={() => void startGoCardlessAuthorization()}
            ...
            {company.isPaymentAccountConnected &&
              company.companyPaymentAccount?.verificationStatus ===
                ...
                <div className="mt-6 rounded-lg border border-blue-100 bg-blue-50/70 p-5">
                  ...
                  <p className="mt-2 text-sm text-zinc-600">
                    GoCardless examine les informations de votre compte. Vous
                    serez averti dès que la vérification sera terminée.
                  </p>
                </div>
              )}
          </section>
          ...
          {company.status === "CLOSED" && (
            <div className="mt-6 rounded-lg border border-red-100 bg-red-50/70 p-5">
              ...
              <p className="mt-2 text-sm text-zinc-600">
                Cette entreprise est conservée en consultation. Ses informations
                ne peuvent plus être modifiées.
              </p>
              {company.closingReason && (
                <p className="mt-3 text-sm text-zinc-700">
                  <span className="font-semibold">Motif de fermeture :</span>{" "}
                  {company.closingReason}
                </p>
              )}
            </div>
