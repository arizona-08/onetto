  ...
  return (
    <div className="mx-auto w-full max-w-6xl p-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="mb-1 text-4xl font-title font-semibold">Mes entreprises</h1>
      ...
      {isLoading ? (
        ...
        <div className="mt-8 rounded-lg border border-dashed border-zinc-300 bg-white p-10 text-center">
          ...
          <h2 className="mt-4 font-title text-lg font-semibold">Aucune entreprise</h2>
        ...
        <>
          ...
          {visibleCompanies.map((company) => {
            ...
            return (
              <li key={company.id} className={`rounded-lg border p-5 ${isClosed ? 'border-red-100 bg-red-50/70' : company.isHidden ? 'border-zinc-200 bg-zinc-50 opacity-60 grayscale' : 'border-zinc-200 bg-white'} ${isActive ? 'ring-1 ring-primary' : ''}`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex min-w-0 gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <h2 className="truncate font-title font-semibold text-zinc-900">{company.name}</h2>
                  ...
                  <div className="relative flex shrink-0 items-start gap-2">
                    ...
                    {isMenuOpen && (
                      <div className="absolute right-0 top-full z-20 mt-1 w-56 rounded-md border border-zinc-200 bg-white p-1">
                        {isOwner && isClosed && (
              ...
              {showHiddenCompanies && (
                <ul className="mt-4 grid gap-4 lg:grid-cols-2">
                  {hiddenCompanies.map((company) => {
                    ...
                    const isClosed = company.status === 'CLOSED';
                    return (
                      <li key={company.id} className={`rounded-lg border p-5 ${isClosed ? 'border-red-100 bg-red-50/70' : 'border-zinc-200 bg-zinc-50 opacity-60 grayscale'} ${isActive ? 'ring-1 ring-primary' : ''}`}>
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex min-w-0 gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary"><Building2 className="h-5 w-5" /></div><div className="min-w-0"><h2 className="truncate font-title font-semibold text-zinc-900">{company.name}</h2><p className="truncate text-sm text-zinc-500">{company.email}</p><p className="mt-1 text-sm text-zinc-500">{company.city}, {company.country}</p></div></div>
                          <div className="relative flex shrink-0 items-center gap-2">
                            ...
                            {isMenuOpen && <div className="absolute right-0 top-full z-20 mt-1 w-56 rounded-md border border-zinc-200 bg-white p-1">
      ...
      {companyToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/40 p-4" role="dialog" aria-modal="true" aria-labelledby="delete-company-title">
          <div className="w-full max-w-md rounded-lg bg-white p-6">
            <h2 id="delete-company-title" className="font-title text-xl font-semibold text-zinc-900">Supprimer cette entreprise ?</h2>
      ...
      {companyToClose && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/40 p-4" role="dialog" aria-modal="true" aria-labelledby="close-company-title">
          <div className="w-full max-w-md rounded-lg bg-white p-6">
            <h2 id="close-company-title" className="font-title text-xl font-semibold text-zinc-900">Fermer cette entreprise ?</h2>
