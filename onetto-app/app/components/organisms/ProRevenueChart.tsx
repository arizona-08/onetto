...
export default function ProRevenueChart({ points }: { points: Array<{ label: string; billed: number; collected: number }> }) {
  ...
  ] }} options={{ responsive: true, maintainAspectRatio: false, interaction: { intersect: false, mode: 'index' }, plugins: { legend: { position: 'bottom', align: 'start', labels: { usePointStyle: true, pointStyle: 'circle', boxWidth: 8, boxHeight: 8, padding: 18, color: '#52525B', font: { size: 12 } } }, tooltip: { backgroundColor: '#FFFFFF', titleColor: '#231942', bodyColor: '#52525B', borderColor: 'rgba(69, 74, 222, 0.22)', borderWidth: 1, cornerRadius: 8, caretSize: 6, caretPadding: 8, padding: 10, titleMarginBottom: 5, displayColors: false, titleFont: { family: 'Poppins', size: 12, weight: 600 }, bodyFont: { family: 'Poppins', size: 12, weight: 500 }, callbacks: { label: (context) => `${context.dataset.label} · ${Number(context.raw).toLocaleString('fr-FR')} €` } } }, scales: { y: { beginAtZero: true, border: { display: false }, ticks: { callback: (value) => `${value} €`, color: '#A1A1AA', font: { size: 11 } }, grid: { color: '#F4F4F5', drawTicks: false } }, x: { border: { display: false }, ticks: { color: '#A1A1AA', font: { size: 11 } }, grid: { display: false } } } }} />;
}
