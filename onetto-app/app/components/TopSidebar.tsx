...
const links = [
  ...
  { name: 'Services', href: '/orders', icon: <Waypoints /> },
  { name: 'Factures', href: '/products', icon: <FileChartColumnIncreasing />  },
          ...
          <ul className="px-2 lg:px-0">
            {links.map((link) => (
              <li key={link.name} className="mb-2">
                <a
                  href={link.href}
                  className="flex items-center gap-3 px-4 py-2 rounded-lg text-zinc-700 hover:bg-zinc-300 transition-colors"
                >
                  {link.icon}
                  {link.name}
                </a>
