  ...
  return (
    <div className="relative min-w-8 flex justify-end cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
      ...
      <div className={`${isOpen ? 'block' : 'hidden'} absolute top-full right-0 bg-white border border-zinc-200 rounded-md shadow-lg mt-1 w-40 p-1 z-10`}>
        <button className="block w-full text-left px-4 py-2 text-sm text-zinc-700 rounded-md hover:bg-zinc-100" onClick={() => triggerEdit && triggerEdit(clientId)}>
          Modifier
        </button>
        <button className="block w-full text-left px-4 py-2 text-sm text-red-500 rounded-md hover:bg-zinc-100" onClick={() => triggerDelete && triggerDelete(clientId)}>
          Suprimer
        </button>
      </div>
    </div>
